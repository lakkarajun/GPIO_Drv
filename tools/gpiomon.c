...
{
    printf("Raju:%s: GPIO:%d, Event:%d\n", __func__, line_offset, event_type);
	if (!ctx->silent) {
		if (ctx->fmt)
    ...
    printf("Raju:%s: Events_Done:%d\n", __func__, ctx->events_done);
	ctx->events_done++;
}
